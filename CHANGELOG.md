...
- `fit_chains_with_threads` and `Sampler::advance_all`: the chains of a
  ...
  rows of a prediction split into that many contiguous chunks, each
  evaluated on a thread of its own by the same operations in the same
  order, so the values do not depend on the count; an execution setting,
