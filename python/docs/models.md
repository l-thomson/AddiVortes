...
`Model.model` selects the observation model. The three below are the published
method: Stone and Gosling (2025) and CRAN AddiVortes.

| `model` | Response | `sigma()` | `predict` returns | `predict_variance` |
| --- | --- | --- | --- | --- |
| `gaussian` | continuous | one sigma per draw | E[y \| x] | sigma^2, constant in x |
| `probit` | labels in {0, 1} | empty | P(y = 1 \| x) | not applicable |
| `heteroscedastic` | continuous | empty | E[y \| x] | s^2(x), varying in x |
...
fitted = Model(model="probit", m=25, burn_in=50, draws=100).fit(x, y, random_state=1)
...
`m_var` inverse-gamma variance tessellations. `predict_variance` gives s^2(x)
per draw.
...
fitted = Model(model="heteroscedastic", m_var=20, m=25, burn_in=50, draws=100).fit(
    x, y, random_state=1
)
...
`metric` sets the distance of each covariate column, one entry per column in
column order. Non-Euclidean columns are not scaled.
