...
as `model.matrix` encodes them, and the default four `chains` carry
...
four chains on one core. As one calibration point, 600 sweeps of
n = 1000 and p = 3 with 100 tessellations take about 7 seconds on one
core of a 2025 laptop. `options(mc.cores = 4)` runs the chains on four
cores for the same draws; the fit then takes less than half the
one-thread time (about 45 per cent at n = 200 on four cores of a 2025
laptop), so it costs under two chains rather than four.
