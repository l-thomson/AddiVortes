...
many rows it costs about what the sweeps cost. It therefore carries the
weight of the sweeps in the bar rather than a step of it: the bar is
around half way when the sweeps end, and it stands until the fit is
complete. Pooling is one call into the core, so the bar rests there
rather than advancing through it. Each phase names itself in the
progression's message, which the `"cli"` and `"progress"` handlers
display and `"txtprogressbar"` does not. Reporting does not change the
draws.
