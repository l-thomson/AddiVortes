...
over the sweeps of every chain, then one for pooling the draws and one
for the convergence summary. Both run after the last sweep, and pooling
predicts at every training row for every kept draw, so on a long
schedule over many rows it outlasts the sweeps; each carries a step of
its own, so the bar stands until the fit is complete. Each phase names
itself in the progression's message, which the `"cli"` and `"progress"`
handlers display and `"txtprogressbar"` does not. Reporting does not
change the draws.
