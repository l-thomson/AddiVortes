...
`plot(fit)` draws the fitted values against the response and the
residuals against the fitted values.

## Documentation

- [`vignette("thiessen")`](https://l-thomson.github.io/thiessen/r/articles/thiessen.md),
  getting started: a fit, its methods, the convergence diagnostics and
  what to do when they warn.
...
- [`vignette("h-addivortes")`](https://l-thomson.github.io/thiessen/r/articles/h-addivortes.md),
  the heteroscedastic model, where the spread varies with x.
- [`vignette("posterior")`](https://l-thomson.github.io/thiessen/r/articles/posterior.md),
  the draws themselves: posterior, loo, bayesplot and tidybayes.
...
- [`vignette("sampler-api")`](https://l-thomson.github.io/thiessen/r/articles/sampler-api.md),
  driving the Gibbs loop from R to build a model the package does not
  ship.
...
[`saveRDS()`](https://rdrr.io/r/base/readRDS.html) writes a fit and a
...
The outcome family carries the prior on the noise:
...
`k` fixes the spread of the cell-value prior, `lambda_c` is the Poisson
rate of the cell-count prior, and the nested
...
hold the centre-proposal scale `sigma_c` and the covariate-inclusion
weight `omega`. Setting `prior_only = TRUE` in
[`general_params()`](https://l-thomson.github.io/thiessen/r/reference/general_params.md)
draws from the prior alone.
[`vignette("control-surface")`](https://l-thomson.github.io/thiessen/r/articles/control-surface.md)
...
a function here, and
[`thiessen_control()`](https://l-thomson.github.io/thiessen/r/reference/thiessen_control.md)
accepts the Gaussian and probit families only.
...
report the core version and the feature setting of the build in use, and
a bug report should carry both. Each of the four graduates on its own
once calibrated, with no build flag to turn on, under the policy in
...
hands over the Gibbs loop one sweep at a time and allows the response to
be rewritten between sweeps, which is what a latent-Gaussian
augmentation needs.
[`vignette("sampler-api")`](https://l-thomson.github.io/thiessen/r/articles/sampler-api.md)
